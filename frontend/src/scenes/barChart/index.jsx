...
const BarChart = () => {
  ...
  const handleExportPdf = () => {
    ...
    // Utilise les données API si dispo, sinon mockData
    const chart =
      stats?.question_types?.labels && stats?.question_types?.counts
        ? {
            labels: stats.question_types.labels,
    ...
    // Transforme en tableau d'objets pour le PDF
    const rows = chart.labels.map((label, i) => ({
      "Question Type": label,
      "Number of Responses": chart.counts[i],
    }));
    ...
    try {
      exportChartDataToPdf({
        ...
        fileName: "bar_chart_data.pdf",
      });

      setSnackbar({
        open: true,
        message: "Chart data exported as PDF!",
        severity: "success",
      });
    } catch (err) {
      setSnackbar({
              ...
              >
                <CardContent>
                  ...
                  <Typography variant="body1" color={colors.grey[100]}>
                    This chart displays the distribution of different question
                    types in our system.
                    {stats?.question_types?.labels &&
                    stats.question_types.labels.length > 0 ? (
                      <>
                        {" "}
                        The most common question type is{" "}
                        <strong>
                          {stats.question_types.labels[0]}
                        </strong> with {stats.question_types.counts[0]}{" "}
                        responses, representing{" "}
                        {Math.round(
                          (stats.question_types.counts[0] /
                  ...
                  <Typography variant="body1" color={colors.grey[100]} mt={2}>
                    Use this information to understand user preferences and
                    optimize your question creation strategy.
                  </Typography>
                </CardContent>
              </Card>
            </motion.div>
